...
# Diese Werte werden über Docker gesetzt, nicht hier im Code.
EVCC_URL = os.environ.get('EVCC_URL', 'http://192.168.178.16:7070')
EVCC_PASSWORD = os.environ.get('EVCC_PASSWORD', '')

# E-Mail Konfiguration
SMTP_SERVER = os.environ.get('SMTP_SERVER', 'mail.gmx.net')
SMTP_PORT = int(os.environ.get('SMTP_PORT', 587))
SENDER_EMAIL = os.environ.get('SENDER_EMAIL')
SENDER_PASSWORD = os.environ.get('SENDER_PASSWORD')
RECIPIENT_EMAIL = os.environ.get('RECIPIENT_EMAIL')

# PDF & Vorlagen-Einstellungen
AUSGABE_ORDNER = '/output' # Wir speichern PDFs in einem dedizierten Ordner im Container
...
def sende_email_mit_anhang(subject, body, anhang_pfad):
    """Baut und versendet eine E-Mail mit dem generierten PDF."""
    ...
    try:
        ...
        server.starttls()  # Sichere Verbindung mit STARTTLS
...
# ... (Die Funktionen 'daten_von_api_holen', 'json_zu_dataframe' bleiben exakt gleich wie in der vorherigen Antwort) ...
# HINWEIS: Aus Platzgründen sind die Funktionen hier nicht erneut eingefügt. 
# Bitte kopieren Sie sie aus der vorherigen Antwort hierher.
...
def erstelle_pdf(df, year, month):
    """Erstellt die PDF-Datei und gibt den Pfad zurück."""
    ...
    env = Environment(loader=FileSystemLoader('.')) # Vorlage liegt im selben Ordner
    ...
    os.makedirs(AUSGABE_ORDNER, exist_ok=True) # Stelle sicher, dass der Ausgabeordner existiert
...
def main():
    ...
    if not json_data:
        print("Konnte keine Daten von der API abrufen. Skript wird beendet.")
        return
